...
BinRouter::Error BinRouter::updateBinNode(std::string_view id, NodePtr node) {
    auto [bin, inserted] = _bins.emplace(id, Bin{{{node}}, PathSearch(PathSearch::Config{std::string(id)})});
    ...
    auto found = std::find_if(path.begin(), path.end(), [&node](auto visit){return visit.node == node;});
    if (found == path.end()) {
